...
export const EnhancedYamlPreview: React.FC<EnhancedYamlPreviewProps> = ({ workflow }) => {
  ...
  const renderYamlWithSyntaxHighlighting = (yaml: string) => {
    if (!highlightSyntax) {
      return <pre className="text-gray-300 font-mono text-sm whitespace-pre-wrap">{yaml}</pre>;
    }
      ...
      <pre className="text-sm font-mono">
        {lines.map((line, index) => {
          ...
          if (line.trim().endsWith(':') && !line.includes('|') && !line.includes('>')) {
            highlightedLine = <span className="text-blue-400 font-semibold">{line}</span>;
          } else if (line.trim().startsWith('- ')) {
            const parts = line.split('- ');
            highlightedLine = (
              <span>
                {parts[0]}<span className="text-yellow-400">- </span>
            ...
            highlightedLine = <span className="text-gray-300">{line}</span>;
          }
  ...
  return (
    <div className="space-y-6">
      <Card className="bg-gray-900/50 backdrop-blur-sm border-gray-700 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            {workflow.filename && (
              <p className="text-sm text-gray-400 mt-1">{workflow.filename}</p>
            )}
          </div>
        ...
        <div className="bg-gray-950 rounded-lg p-4 overflow-auto max-h-96 min-h-[400px]">
          {workflow.yaml ? (
      ...
      {workflow.instructions.length > 0 && (
        <Card className="bg-blue-900/20 backdrop-blur-sm border-blue-700 p-6">
          ...
          <div className="space-y-2">
            {workflow.instructions.map((instruction, index) => (
              <div key={index} className="flex items-start space-x-3">
                <Badge variant="secondary" className="bg-blue-600 text-white text-xs mt-0.5">
                  {index + 1}
                </Badge>
