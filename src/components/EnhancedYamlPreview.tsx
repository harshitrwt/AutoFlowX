...
export const EnhancedYamlPreview: React.FC<EnhancedYamlPreviewProps> = ({ workflow }) => {
  ...
  const renderYamlWithSyntaxHighlighting = (yaml: string) => {
    if (!highlightSyntax) {
      return <pre className="text-gray-700 dark:text-gray-300 font-mono text-sm whitespace-pre-wrap">{yaml}</pre>;
    }
      ...
      <pre className="text-sm font-mono">
        {lines.map((line, index) => {
          ...
          if (line.trim().endsWith(':') && !line.includes('|') && !line.includes('>')) {
            highlightedLine = <span className="text-orange-600 dark:text-orange-400 font-semibold">{line}</span>;
          } else if (line.trim().startsWith('- ')) {
            const parts = line.split('- ');
            highlightedLine = (
              <span>
                {parts[0]}<span className="text-orange-500 dark:text-orange-400">- </span>
            ...
            highlightedLine = <span className="text-gray-500 dark:text-gray-400 italic">{line}</span>;
          } else {
            highlightedLine = <span className="text-gray-700 dark:text-gray-300">{line}</span>;
          }

          return (
            <div key={index} className="flex">
              {showLineNumbers && (
                <span className="text-gray-400 dark:text-gray-500 text-xs mr-4 select-none w-8 text-right">
                  {index + 1}
                </span>
              )}
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-professional p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-xl font-bold text-gray-900 dark:text-white">Generated Pipeline</h3>
            {workflow.filename && (
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{workflow.filename}</p>
            )}
          </div>
        ...
        <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 overflow-auto max-h-96 min-h-[400px]">
          {workflow.yaml ? (
          ...
          <div className="space-y-3">
            {workflow.instructions.map((instruction, index) => (
              <div key={index} className="flex items-start space-x-3">
                <div className="bg-orange-600 text-white text-xs font-bold rounded-full w-6 h-6 flex items-center justify-center mt-0.5">
                  {index + 1}
                </div>
                <p className="text-orange-800 dark:text-orange-200 text-sm leading-relaxed">{instruction}</p>
              </div>
            ))}
          </div>
        </div>
